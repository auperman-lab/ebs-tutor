...
export const useURLQuery = () => {
	...
	const getParams = (): ParamsType => {
		return {
			...
			sort: (searchParams.get('sort') as ParamsType['sort']) || defaultParams.sort,
			category: searchParams.get('category')?.split(',').map(Number) || undefined,
	...
	const setParams = (params: ParamsType) => {
		Object.entries(params).forEach(([key, value]) => {
			...
			} else if (!value || value === 'all' || value === defaultParams.sort || value === defaultParams.page) {
				searchParams.delete(key);
			} else {
