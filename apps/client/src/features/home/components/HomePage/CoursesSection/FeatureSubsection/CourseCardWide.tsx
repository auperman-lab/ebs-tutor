...
import { formatOldPrice, formatPrice } from '@client/utils';
...
export const CourseCardWide = ({
  ...
  return (
    <Flex onClick={onClick} className={styles.cardWrapper}>
      {imageUrl ? (
        <img className={styles.cardImage} src={imageUrl} />
      ) : (
        <NoImage className={styles.cardEmptyImage} />
      )}
      ...
      <Flex vertical gap={17} className={styles.infoWrapper}>
        <Flex vertical gap={8}>
          <Flex
            ...
            align="center"
            className={styles.pricetagWrapper}
          >
