...
export const Sidebar = () => {
	...
	const getOldPrice = (): string => {
		if (course?.product.price_old != null && course?.product.price_old !== course?.product.price) {
	...
	const getDiscount = (): string => {
		if (course?.product.price_old != null && course?.product.price_old !== course?.product.price) {
			return `${(course?.product.price_old - course?.product.price) * 100 / course?.product.price_old}%`;
		}
		return '';
	};
		...
		<Flex vertical gap={24} className={styles.sidebarWrapper}>
			...
			<Flex vertical gap={16}>
				<Flex justify="space-between" align="center">
					...
					<div className={styles.textSecondary}>{course?.duration ?? 'Forever'}</div>
				</Flex>
				<Flex justify="space-between" align="center">
					<Flex gap={8} align="center" justify="start">
				...
				<Flex justify="space-between" align="center">
					...
					<div className={styles.textSecondary}>{course?.users_count ?? 'over 9000'}</div>
				</Flex>
				<Flex justify="space-between" align="center">
					<Flex gap={8} align="center" justify="start">
					...
					<div className={styles.textSecondary}>{course?.language ?? 'Latin'}</div>
				</Flex>
			...
			<Flex vertical gap={12}>
				...
				<Flex gap={12}>
					<Button size="large" className={styles.mediumButton} type="default">Add To Wishlist</Button>
					<Button size="large" className={styles.mediumButton} type="default">Gift Course</Button>
				</Flex>
				<Flex
					align="center"
					justify="center"
			...
			<Flex gap={18} vertical>
				...
				<Flex gap={8}>
					<Button size="large" className={styles.smallButton} onClick={onCopyLink}>
						Copy link
					</Button>
					<Button size="large" className={styles.smallButton} icon={<Envelope />} onClick={onEmailShare} />
					<Button size="large" className={styles.smallButton} icon={<Twitter />} onClick={onTwitterShare} />
					<Button size="large" className={styles.smallButton} icon={<Facebook />} onClick={onFacebookShare} />
					<Button size="large" className={styles.smallButton} icon={<Linkedin />} onClick={onLinkedInShare} />
				</Flex>
