...
export const TutorPage = () => {
	...
	return (
		<Row gutter={[24, 24]} className={styles.wrapper}>
			<Col span={24}>
				<TutorPageHeader name={`${tutor?.first_name} ${tutor?.last_name}`} image={tutor?.url_avatar} email={tutor?.email} coursesAmount={courses?.total}/>
			</Col>
			<Col span={8}>
			...
			<Col span={16}>
				<Flex gap={24} vertical>
					<div className={styles.courseTitle}>{tutor?.first_name} Courses ({courses?.total})</div>
					{
						isLoading
							? <Spin size="large" />
							:
